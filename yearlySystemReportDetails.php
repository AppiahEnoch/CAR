...
$finalFileName='report/systemYearlyReport.pdf'; 



$pageTitle="SYSTEM YEARLY REPORT";
...
$sql2 = "SELECT 
DATE_FORMAT(recdate, CONCAT(DAY(recdate), 
CASE 
    WHEN DAY(recdate) IN (11,12,13) THEN 'th'
    WHEN DAY(recdate) % 10 = 1 THEN 'st'
    WHEN DAY(recdate) % 10 = 2 THEN 'nd'
    WHEN DAY(recdate) % 10 = 3 THEN 'rd'
    ELSE 'th'
END, ' %b, %Y')) AS 'Date', location,
locationUser AS Manager,
washer,
DATE_FORMAT(recdate, '%Y') AS 'YEAR', 
...
FROM washed 
WHERE 
recdate BETWEEN DATE_SUB(NOW(), INTERVAL $sysDays YEAR) AND NOW() 
ORDER BY recdate, `location` ASC";
